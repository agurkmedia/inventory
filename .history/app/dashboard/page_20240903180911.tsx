...
export default function Dashboard() {
  ...
  const [step, setStep] = useState(0); // 0: not started, 1: inventory, 2: item
  ...
  useEffect(() => {
    ...
    router.events.on('routeChangeComplete', fetchDashboardData);

    return () => {
      router.events.off('routeChangeComplete', fetchDashboardData);
    };
  }, [router]);
      ...
      <div className="flex items-center justify-center h-full">
        ...
        >
          <h2 className="text-2xl font-bold text-white mb-4">
            {step === 1 ? "Create Your First Inventory" : "Add Your First Item"}
          </h2>
          <p className="text-indigo-200 mb-6">
            {step === 1 
              ? "Start by creating an inventory to organize your items." 
              : "Now that you have an inventory, let's add your first item."}
          </p>
          <Link 
            href={step === 1 ? "/dashboard/inventories/new" : "/dashboard/items/new"}
          ...
          >
            {step === 1 ? "Create Inventory" : "Add Item"}
          </Link>
        </motion.div>
      </div>
    );
