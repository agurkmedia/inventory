    ...
    <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
      {isEditing ? (
        ...
        <>
          ...
          <p className="text-sm text-indigo-200 mb-3">Type: {inventory.isItemScraping ? 'Item Scraping' : 'Normal'}</p>
          <div className="flex flex-col space-y-2">
            <Link href={inventory.isItemScraping ? `/dashboard/itemscrapings/${inventory.id}` : `/dashboard/inventories/${inventory.id}`} className="text-indigo-400 hover:text-indigo-300 text-sm">
              View Details
            </Link>
            <Link href={inventory.isItemScraping ? `/dashboard/itemscrapings/new?inventoryId=${inventory.id}` : `/dashboard/items/new?inventoryId=${inventory.id}`} className="text-green-400 hover:text-green-300 text-sm">
              Add Item
            </Link>
