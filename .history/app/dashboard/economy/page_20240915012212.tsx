...
export default function EconomyAndBudget() {
  ...
  // Add this in the component to debug the categorySummary state
useEffect(() => {
  console.log('Updated categorySummary:', categorySummary);
}, [categorySummary]);
  useEffect(() => {
    ...
    <div className="space-y-4 h-full">
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <p className={`text-xl font-bold ${categorySummary.balance >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            Overall Balance: ${categorySummary.balance.toFixed(2)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>{renderPieChart(categorySummary.incomes.breakdown, 'Income Breakdown')}</div>
          <div>{renderPieChart(categorySummary.expenses.breakdown, 'Expense Breakdown')}</div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div>{renderBarChart(categorySummary.incomes.breakdown, 'Income Breakdown', '#4CAF50')}</div>
          <div>{renderBarChart(categorySummary.expenses.breakdown, 'Expense Breakdown', '#F44336')}</div>
        </div>
      </div>
