...
export default function EconomyAndBudget() {
  ...
  const handleViewChange = (mode: 'monthly' | 'yearly') => {
    setViewMode(mode);
    if (mode === 'yearly') {
      fetchYearlyData(selectedMonth.getFullYear());
    } else {
      fetchMonthData(selectedMonth);
      fetchCategorySummary(selectedMonth);
    }
  ...
  const renderBarChart = (data: { [key: string]: number } | Array<{ source: string; amount: number }>, title: string, color: string) => {
    let labels: string[];
    let values: number[];
  ...
  return (
    <div className="space-y-4 h-full">
      ...
      {/* View mode and date range picker */}
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <p className={`text-white text-xl font-bold ${categorySummary.balance >= 0 ? 'text-green-500' : categorySummary.balance > -1000 ? 'text-red-500' : 'text-red-700'}`}>Overall Balance: ${categorySummary.balance.toFixed(2)}</p>
        </div>
      </div>
