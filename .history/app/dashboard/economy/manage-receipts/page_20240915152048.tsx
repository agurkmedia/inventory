...
const fetchExistingReceiptItems = async () => {
  ...
  const parseCSV = (results: Papa.ParseResult<any>, columnMapping: CSVModel['columnMapping']) => {
    return results.data.map((record: any) => {
      ...
      const outAmount = parseFloat(record[columnMapping.outAmount]?.replace(',', '.') || '0');
      const inAmount = parseFloat(record[columnMapping.inAmount]?.replace(',', '.') || '0');
