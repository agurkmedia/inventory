...
const fetchExistingReceiptItems = async () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Fetch the latest existing receipt items from the database before filtering
    await fetchExistingReceiptItems();
        ...
        complete: (results) => {
          try {
            ...
            const newTransactions = parsed.filter((transaction) => {
              const groupKey = transaction.description.trim();
  
              // Debugging: Show group key
              console.log(`Processing groupKey: ${groupKey}`);
  
              if (processedGroups.includes(groupKey)) {
                console.log(`Group "${groupKey}" is already processed.`);
                return false;
              }
  
              // Check if the transaction already exists in the database
              const exists = existingReceiptItems.some((item) => {
                const transactionDate = new Date(transaction.date).toISOString().split('T')[0];
                const itemDate = new Date(item.date).toISOString().split('T')[0];
  
                // Debugging: Log comparison details
                console.log('Comparing parsed transaction to existing item:');
                ...
                const descriptionMatches = item.item.name === transaction.description.trim();
                const dateMatches = itemDate === transactionDate;
                const amountMatches = item.totalPrice === Math.abs(transaction.amount);
                ...
                return descriptionMatches && dateMatches && amountMatches;
              });
  
              console.log(`Transaction exists: ${exists}`);
              return !exists; // Include only if it doesn't exist in the database
            });
  
            // Debugging: Log new transactions after filtering
            console.log('New transactions after filtering:', newTransactions);
