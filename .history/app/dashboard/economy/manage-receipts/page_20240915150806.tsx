...
const DNBModel: CSVModel = {
  ...
  parseFunction: (results: Papa.ParseResult<any>) => {
    return results.data.map((record: any) => {
      ...
      // Convert the raw date to yyyy-mm-dd format for consistency
      ...
      const amount = inAmount - outAmount; // Preserve the sign for accurate calculations

      return {
        date: parsedDate,
        description: record['Forklaring'],
        amount: amount, // Use the calculated amount with its original sign
      };
    });
  },
};
...
export default function ManageReceipts() {
  ...
  // Fetch receipts including receipt items
...
const [showExistingItems, setShowExistingItems] = useState(false); // State to toggle view
...
const fetchExistingReceiptItems = async () => {
  try {
    ...
    const data = await res.json();
    console.log('Fetched existing receipt items:', data); // Debugging line
  ...
  const [processedGroups, setProcessedGroups] = useState<string[]>([]); // Track saved groups by their description
      ...
      Papa.parse(file, {
        complete: async (results) => {
          try {
            ...
            // Debugging: Log all parsed transactions
            console.log('Parsed transactions:', parsed);
  
            // Send parsed transactions to the server for filtering
            ...
            // Get the filtered transactions from the server
            const filteredTransactions = await response.json();
            
            // Debugging: Log filtered transactions returned from the server
  ...
  const handleSaveGrouping = async (groupKey: string) => {
    try {
      ...
      const receiptData = {
        storeName: groupKey,
        totalAmount: group.transactions.reduce((acc, transaction) => acc + transaction.amount, 0), // Sum the original amounts to keep the sign
        ...
        items: group.transactions.map((transaction: ParsedTransaction) => {
          ...
          return {
            ...
            quantity: 1,
            totalPrice: transaction.amount, // Use the parsed amount directly, preserving the sign
      ...
      fetchReceipts(); // Refresh receipts to reflect the latest state
      setError('');
    ...
    <div className="mt-8">
      ...
      {showKeywordMappings && (
        ...
        <div className="bg-white bg-opacity-10 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-white mb-2">Add/Edit Keyword Mapping</h3>
          <form onSubmit={handleAddOrUpdateKeywordMapping} className="flex space-x-2">
            <input
              type="text"
              placeholder="Keyword"
              value={newKeyword}
              onChange={(e) => setNewKeyword(e.target.value)}
              className="flex-1 px-3 py-2 text-white bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <input
              type="text"
              placeholder="Description"
          ...
          <h3 className="text-lg font-semibold text-white mt-4 mb-2">Existing Keyword Mappings</h3>
          <ul className="space-y-2">
            {keywordMappings.map((mapping) => (
              <li key={mapping.id} className="flex items-center justify-between bg-gray-800 p-2 rounded-md">
                <div>
                  <span className="font-semibold">{mapping.keyword}</span> - {mapping.description}
                </div>
                <div className="flex space-x-2">
                  <button
