...
const DNBModel: CSVModel = {
  ...
  parseFunction: (results: Papa.ParseResult<any>) => {
    return results.data.map((record: any) => {
      ...
      // Determine the amount correctly
      const amountOut = parseFloat(record['Ut fra konto'] || '0'); // Money going out (expenses)
      const amountIn = parseFloat(record['Inn på konto'] || '0');  // Money coming in (income)

      // If there is an outgoing amount, make it negative; otherwise, use the incoming amount (which is positive)
      const amount = amountOut > 0 ? -amountOut : amountIn;

      return {
        date: parsedDate,
        description: record['Forklaring'],
        amount: amount, // Correctly preserve the sign for expenses and income
      };
    });
  },
};
