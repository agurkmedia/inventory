...
export default function ManageReceipts() {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      ...
      fetchExistingReceiptItems(); // Fetch existing receipt items
    }
  }, [status]);
...
const fetchExistingReceiptItems = async () => {
  ...
  <button
    ...
    className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out mb-4"
  >
    ...
    <div>
      ...
      {Object.keys(groupedTransactions).map((groupKey) => (
        <div key={groupKey} className="bg-white bg-opacity-10 p-4 rounded-lg mb-4">
          ...
          <p className="text-gray-300 mb-2">Total Amount: {groupedTransactions[groupKey].totalAmount.toFixed(2)} NOK</p>

          {/* Only display transactions not in the database, unless 'showExistingItems' is true */}
