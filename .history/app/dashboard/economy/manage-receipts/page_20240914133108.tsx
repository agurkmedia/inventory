...
export default function ManageReceipts() {
  ...
  const handleSaveGrouping = async (groupKey: string) => {
    try {
      ...
      console.log('Fetching or creating items:', { descriptions, inventoryId: receiptsInventoryId });
      const items = await fetchOrCreateItems(descriptions, receiptsInventoryId || '');
      console.log('Fetched or created items:', items);
      ...
      console.log('Receipt saved successfully:', receiptResponse);
  ...
  const fetchOrCreateItems = async (names: string[], inventoryId: string) => {
    try {
      console.log('Fetching or creating items:', { names, inventoryId });
      ...
      const data = await res.json();
      console.log('Fetched or created items:', data);
      return data;
    } catch (err) {
  ...
  const createExpenseCategory = async (name: string): Promise<ExpenseCategory> => {
    console.log('Creating new expense category:', name);
    const res = await fetch('/api/expense-categories', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.log('Created new expense category:', newCategory);
    setExpenseCategories([...expenseCategories, newCategory]);
    return newCategory;
  };
