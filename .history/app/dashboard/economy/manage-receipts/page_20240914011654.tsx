...
interface ReceiptItem {
  ...
  date: string; // Include date in the interface
}
...
export default function ManageReceipts() {
  ...
  // Memoize the keywordMap to improve performance
  ...
  const handleSaveGrouping = async (groupKey: string) => {
    try {
      ...
      fetchReceipts();
      setError('');
      setSuccessMessage({successMessage && <p className="text-green-500 mt-4">{successMessage}</p>});
    } catch (err: any) {
