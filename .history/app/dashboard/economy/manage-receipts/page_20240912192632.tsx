...
export default function ManageReceipts() {
  ...
  const ensureItemInInventory = async (description: string, inventoryId: string) => {
    console.log(`Checking for item with description: ${description} in inventory: ${inventoryId}`);
  ...
  const createReceiptForTransaction = async (transaction: ParsedTransaction, categoryId: string, storeName: string) => {
    try {
      // Convert the transaction.date from "dd.mm.yyyy" to "yyyy-mm-ddT00:00:00Z"
      const [day, month, year] = transaction.date.split('.');  // Split the "dd.mm.yyyy" string
      const isoDate = `${year}-${month}-${day}T00:00:00Z`;  // Create the "yyyy-mm-ddT00:00:00Z" format
      
      const transactionDate = new Date(isoDate);
      ...
      const receiptData = {
        ...
        date: transactionDate.toISOString(),  // Correct ISO date
        categoryId,
      };
