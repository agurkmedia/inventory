...
export default function ManageReceipts() {
  ...
  const handleSaveGrouping = async (groupKey: string) => {
    try {
      ...
      const categoryId = selectedCategories[groupKey]; // Expense category
  
      if (!categoryId) {
        throw new Error('Please select a category before saving.');
      }
  
      // Ensure Receipts inventory exists
      if (!receiptsInventoryId) {
        await ensureReceiptsInventory(); // Make sure the "Receipts" inventory is present
      }
  
      // Ensure the item is in the inventory (if not, create it)
      const itemDescription = group.transactions[0].description; // Assuming all descriptions are the same in the group
      const item = await ensureItemInInventory(itemDescription, receiptsInventoryId);
  
      // Process each transaction and create a receipt/receipt item
      ...
      setError(`Failed to save grouping "${groupKey}". Please try again. ${err.message}`);
    }
  };
