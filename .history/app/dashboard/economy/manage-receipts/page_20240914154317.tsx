      ...
      Papa.parse(file, {
        complete: (results) => {
          try {
            ...
            // Filter out transactions that already exist in the database or belong to processed groups
            const newTransactions = parsed.filter((transaction) => {
              const groupKey = transaction.description.trim();
  
              // Skip the entire group if it has been processed
              if (processedGroups.includes(groupKey)) return false;
  
              // Otherwise, filter out individual transactions that already exist
