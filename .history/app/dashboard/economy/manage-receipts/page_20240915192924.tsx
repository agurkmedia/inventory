...
export default function ManageReceipts() {
  ...
  const fetchReceiptsInventoryId = async () => {
    try {
      const res = await fetch('/api/inventories/receipts');
      if (!res.ok) throw new Error('Failed to fetch Receipts inventory');
      const data = await res.json();
      setReceiptsInventoryId(data.id);
    } catch (err) {
      console.error('Failed to fetch Receipts inventory:', err);
      setError('Failed to load Receipts inventory. Please try again.');
    }
  };

  const handleAddKeywordMapping = async (e: React.FormEvent) => {
      // First, check if we already have a receiptsInventoryId
      if (receiptsInventoryId) {
        return; // If we do, no need to fetch or create
      }

      // Fetch all inventories
      const res = await fetch('/api/inventories');
      if (!res.ok) throw new Error('Failed to fetch inventories');
      const inventories: Inventory[] = await res.json();

      // Look for an existing "Receipts" inventory
      const receiptsInventory = inventories.find(inv => inv.name === 'Receipts');

      if (receiptsInventory) {
        // If found, set the ID and return
        setReceiptsInventoryId(receiptsInventory.id);
      } else {
        // If not found, create a new one
        const createRes = await fetch('/api/inventories', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: 'Receipts' }),
        });

        if (!createRes.ok) throw new Error('Failed to create Receipts inventory');
        const newInventory = await createRes.json();
