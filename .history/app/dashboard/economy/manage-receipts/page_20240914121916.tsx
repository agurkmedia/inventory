...
// Add the state to handle the toggle for showing existing items
const [showExistingItems, setShowExistingItems] = useState(false); // Toggle state for existing items
            ...
            <table className="min-w-full bg-gray-800 rounded-lg overflow-hidden">
              ...
              <tbody>
                {groupedTransactions[groupKey].transactions
                  .filter((transaction) => {
                    if (showExistingItems) return true; // Show all if toggled on
                    // Filter out existing items if toggled off
                    return !existingReceiptItems.some(
                      (item) =>
                        item.item.name === transaction.description.trim() &&
                        item.date === transaction.date &&
                        item.totalPrice === Math.abs(transaction.amount)
