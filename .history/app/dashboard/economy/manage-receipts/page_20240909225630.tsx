...
export default function ManageReceipts() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsed: ParsedReceipt[] = lines.slice(1).map(line => {
      ...
      console.log('Parsed receipts:', parsed);
  ...
  const parseItems = (itemsString: string): { name: string; quantity: number; price: number }[] => {
    ...
    try {
      ...
      // If JSON parsing fails, try to parse as a string with a custom format
      // Assuming items are separated by semicolons and item details by commas
