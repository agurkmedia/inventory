    ...
    Array.from(files).forEach((file) => {
      Papa.parse(file, {
        complete: async (results) => {
          try {
            const selectedModel = csvModels.find(model => model.id === selectedModelId);
            if (!selectedModel) throw new Error('No CSV model selected');

            const parsed = parseCSV(results, selectedModel.columnMapping);
