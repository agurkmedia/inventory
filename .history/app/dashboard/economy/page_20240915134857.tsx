...
export default function EconomyAndBudget() {
  ...
  const renderReceiptStackedBarChart = () => {
    if (!categorySummary.data || categorySummary.data.length === 0 || !categorySummary.data[0].receipts.breakdown) {
      return <p className="text-white">No receipt data available</p>;
    }

    const receiptBreakdown = categorySummary.data[0].receipts.breakdown;
    const categories = Object.keys(receiptBreakdown);
    const amounts = Object.values(receiptBreakdown).map(amount => Math.abs(amount as number));
    ...
    const options = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Receipt Breakdown Summary',
          color: 'white',
          font: { size: 16 },
        },
      },
