...
export default function EconomyAndBudget() {
  ...
  const fetchYearlyData = async (year: number) => {
    try {
      const res = await fetch(`/api/balances/yearly-summary?year=${year}`);
      if (!res.ok) throw new Error('Failed to fetch yearly summary');
      const data = await res.json();
      setCategorySummary(data);
      // You might need to adjust this based on the structure of your yearly data
  ...
  const handleViewChange = (mode: 'daily' | 'monthly' | 'yearly') => {
    setViewMode(mode);
    if (mode === 'yearly') {
    ...
    // For 'daily' view, you might want to implement a similar function to fetch daily data
  };
  ...
  const renderPieChart = (data: { [key: string]: number } | Array<{ source: string; amount: number; date: string }>, title: string) => {
  ...
  const renderReceiptBreakdown = () => {
    // Convert the breakdown object to an array
    const receiptItemsArray = Object.entries(categorySummary.receipts.breakdown).map(
      ([category, data]: [string, any]) => ({
        category,
        date: data.date || 'N/A', // Assuming each entry might have a date, if not, use 'N/A'
        amount: data.amount || 0, // Assuming each entry might have an amount, if not, use 0
      })
    );
  
    return receiptItemsArray.map((item, index) => (
      <tr key={index}>
  ...
  const renderBarChart = (data: { [key: string]: number } | Array<{ source: string; amount: number; date: string }>, title: string, color: string) => {
    if (!data || (Array.isArray(data) ? data.length === 0 : Object.keys(data).length === 0)) {
      return <p className="text-white">No data available</p>;
    }
  ...
  return (
    <div className="space-y-4 h-full">
      ...
      {/* View mode buttons and date range picker */}
      <div className="flex flex-wrap items-center gap-4 mb-4">
        <button
          onClick={() => handleViewChange('daily')}
          className={`px-4 py-2 rounded ${viewMode === 'daily' ? 'bg-blue-600' : 'bg-blue-400'} text-white`}
        >
          Daily
        </button>
        <button
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center">
          ...
          <div>
            {viewMode === 'yearly' ? (
              <>
                <button onClick={handlePreviousYear} className="bg-blue-500 text-white px-2 py-1 rounded mr-2">Previous Year</button>
                <span className="text-white">{selectedYear}</span>
                <button onClick={handleNextYear} className="bg-blue-500 text-white px-2 py-1 rounded ml-2">Next Year</button>
              </>
            ) : (
              <>
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <div className="flex items-center space-x-4">
            <div>
              <p className="text-green-400 font-semibold">Total Income: ${categorySummary.incomes.total.toFixed(2)}</p>
              <p className="text-red-400 font-semibold">Total Expenses: ${categorySummary.expenses.total.toFixed(2)}</p>
            </div>
            <p className={`text-xl font-bold ${
              categorySummary.balance >= 0 ? 'text-green-400' : (categorySummary.balance < -1000 ? 'text-red-600' : 'text-red-400')
            }`}>
              Overall Balance: ${categorySummary.balance.toFixed(2)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="h-80">{renderPieChart(categorySummary.incomes.breakdown, 'Income Breakdown')}</div>
          <div className="h-80">{renderPieChart(categorySummary.expenses.breakdown, 'Expense Breakdown')}</div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          <div className="h-80">{renderBarChart(categorySummary.incomes.breakdown, 'Income Breakdown', '#4CAF50')}</div>
          <div className="h-80">{renderBarChart(categorySummary.expenses.breakdown, 'Expense Breakdown', '#F44336')}</div>
        </div>
      </div>
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-white">{viewMode === 'yearly' ? 'Yearly Balance' : 'Daily Balance'}</h2>
          <div>
            {viewMode === 'yearly' ? (
              <>
                <button onClick={handlePreviousYear} className="bg-blue-500 text-white px-2 py-1 rounded mr-2">Previous Year</button>
      ...
      {/* Balance Table */}
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <h2 className="text-lg font-semibold text-white mb-2">{viewMode === 'yearly' ? 'Yearly Balance Table' : 'Daily Balance Table'}</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full text-white">
            <thead>
              <tr>
