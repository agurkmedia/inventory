...
export default function ImportPDF() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      <h1 className="text-3xl font-bold mb-4">Import PDF</h1>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md">
        ...
        <div className="mb-4">
          ...
          <select
            id="method"
            value={method}
            onChange={(e) => setMethod(e.target.value as 'pdf' | 'ocr')}
            className="mt-1 block w-full text-black bg-white"
          >
            <option value="ocr">OCR (Norwegian)</option>
            <option value="pdf">PDF Parsing</option>
          </select>
        </div>
      ...
      {loading && (
        <div className="mt-4">
          <p>Processing PDFs... (OCR may take a while)</p>
        </div>
      )}
      ...
      {!loading && Object.keys(groupedTransactions).length > 0 && (
        <div className="mt-4">
          <h2 className="text-xl font-bold mb-2">Grouped Transactions</h2>
          {Object.keys(groupedTransactions).map((groupKey) => (
            <div key={groupKey} className="mb-4">
              <h3 className="text-lg font-bold mb-2">{groupKey}</h3>
              <table className="table-auto w-full">
                <thead>
                  <tr className="bg-white">
                    <th className="px-4 py-2 text-black">Date</th>
                ...
                <tbody>
                  {groupedTransactions[groupKey].transactions.map((t: ParsedTransaction, index: number) => (
                    <tr key={index}>
                      <td className="border px-4 py-2">{t.date}</td>
                      <td className="border px-4 py-2">{t.description}</td>
                      <td className="border px-4 py-2">{t.amount.toFixed(2)}</td>
              ...
              <div className="mt-2">
                <span className="font-bold">Total Amount:</span> {groupedTransactions[groupKey].totalAmount.toFixed(2)}
              </div>
              <div className="mt-2">
                <label htmlFor={`category-${groupKey}`} className="block text-sm font-medium text-gray-700">
                  Select Category
                </label>
                <select
                  id={`category-${groupKey}`}
                  value={selectedCategories[groupKey] || ''}
                  onChange={(e) => setSelectedCategories({ ...selectedCategories, [groupKey]: e.target.value })}
                  className="mt-1 block w-full text-black bg-white"
                >
      ...
      {showKeywordMappings && (
        <div className="mt-4">
          <h2 className="text-xl font-bold mb-2">Keyword Mappings</h2>
          <table className="table-auto w-full">
            <thead>
              <tr className="bg-white">
                <th className="px-4 py-2 text-black">Keyword</th>
                <th className="px-4 py-2 text-black">Description</th>
      ...
      <div className="mt-4">
        <h2 className="text-xl font-bold mb-2">Add/Edit Keyword Mapping</h2>
        <form onSubmit={editingMapping ? handleUpdateKeywordMapping : handleAddKeywordMapping} className="w-full max-w-md">
          <div className="mb-4">
      ...
      {!isYearConfirmed && (
        <div className="mt-4">
          <h2 className="text-xl font-bold mb-2">Set Year for All Transactions</h2>
          <input
            type="text"
            value={year}
          ...
          />
          <button
            onClick={() => setIsYearConfirmed(true)}
            className="mt-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          >
            Confirm Year
          </button>
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const file of files) {
      ...
      if (method === 'pdf') {
        ...
        extractedText = await extractTextUsingOCR(file);
      }
