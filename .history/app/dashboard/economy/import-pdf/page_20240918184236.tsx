  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFile(null);
        fetchTransactions(); // Fetch updated transactions after successful import
      } else {
        setMessage(`Error: ${result.error || 'Unknown error occurred'}`);
      }
    } catch (error) {
      console.error('Error importing PDF:', error);
      setMessage('An error occurred while importing the PDF. Please check the console for more details.');
    } finally {
      setUploading(false);
    }
  };
