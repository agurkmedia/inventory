...
export default function EconomyAndBudget() {
  ...
  const renderReceiptBreakdown = () => {
    // Ensure that the receipt breakdown is an array of objects
    const receiptData = Object.entries(categorySummary.receipts.breakdown).map(
        ([category, amount]) => ({
            category,
            amount,
            date: new Date() // Placeholder; update with actual receipt item date if available
        })
    );

    return receiptData.map((item, index) => (
        <tr key={index}>
