...
interface CategorySummary {
  ...
  data: Array<{
    incomes: { total: number; breakdown: { [key: string]: number } };
    expenses: { total: number; breakdown: { [key: string]: number } };
    receipts: { total: number; breakdown: { [key: string]: number } };
    balance: number;
    startDate?: string;
    endDate?: string;
    totalDays?: number;
...
export default function EconomyAndBudget() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>({ mode: 'monthly', label: 'Monthly' });
  ...
  const fetchCategorySummary = async (date: Date, viewMode: ViewMode) => {
    try {
      let url = '/api/balances/category-summary?';
      
      if (viewMode.mode === 'monthly') {
        url += `month=${date.getMonth() + 1}&year=${date.getFullYear()}&mode=monthly`;
      } else if (viewMode.mode === 'yearly') {
        url += `year=${date.getFullYear()}&mode=yearly`;
      } else if (viewMode.mode === 'last12months') {
        const endDate = new Date();
        const startDate = new Date(endDate);
        startDate.setFullYear(startDate.getFullYear() - 1);
        url += `startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}&mode=last12months`;
      } else if (viewMode.mode === 'allTime') {
        url += `mode=allTime`;
      }
  ...
  const fetchYearlyData = async (year: number) => {
    try {
      ...
      const summarizedData: CategorySummary = {
        mode: 'yearly',
        data: [{
      ...
      yearlyData.forEach((monthData) => {
        summarizedData.data[0].incomes.total += monthData.incomes.total;
        summarizedData.data[0].expenses.total += monthData.expenses.total;
        summarizedData.data[0].receipts.total += monthData.receipts.total;

        // Aggregate income breakdown
        Object.entries(monthData.incomes.breakdown).forEach(([source, amount]) => {
          summarizedData.data[0].incomes.breakdown[source] = (summarizedData.data[0].incomes.breakdown[source] || 0) + (amount as number);
        });

        // Aggregate expense breakdown
        Object.entries(monthData.expenses.breakdown).forEach(([category, amount]) => {
          summarizedData.data[0].expenses.breakdown[category] = (summarizedData.data[0].expenses.breakdown[category] || 0) + (amount as number);
        });

        // Aggregate receipts breakdown
        Object.entries(monthData.receipts.breakdown).forEach(([category, amount]) => {
          summarizedData.data[0].receipts.breakdown[category] = (summarizedData.data[0].receipts.breakdown[category] || 0) + (amount as number);
        });
      });

      summarizedData.data[0].balance = summarizedData.data[0].incomes.total - summarizedData.data[0].expenses.total;
  ...
  const handleViewChange = (newMode: ViewMode) => {
    setViewMode(newMode);
    fetchCategorySummary(selectedMonth, newMode);
  };
  ...
  const renderPieChart = (data: any, title: string) => {
    ...
    const chartData = {
      ...
      datasets: [
        {
          data: Object.values(data).map(Math.abs), // Use absolute values for pie chart
          backgroundColor: [
    ...
    const options = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `${title} (Total: $${Object.values(data).reduce((sum, val) => sum + (val as number), 0).toFixed(2)})`,
          color: 'white',
          font: { size: 16 },
        },
      },
  ...
  const renderReceiptStackedBarChart = () => {
    ...
    let amounts = Object.values(expenseBreakdown).map(amount => amount as number);
    ...
    const options = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Expense Breakdown Summary (Total: $${sortedAmounts.reduce((sum, val) => sum + val, 0).toFixed(2)})`,
          color: 'white',
          font: { size: 16 },
        },
      },
    ...
    <div className="space-y-4 h-full">
      ...
      <div className="flex gap-4 mb-4">
        {([
          ...
          <button
            ...
            className={`px-4 py-2 rounded ${viewMode.mode === mode.mode ? 'bg-blue-600' : 'bg-blue-400'} text-white`}
          >
            {mode.label}
          </button>
        ))}
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold text-white">{viewMode.label === 'Yearly' ? 'Yearly Summary' : 'Current Month Summary'}</h2>
          <div>
            {viewMode.mode === 'yearly' ? (
              <>
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleSection('financialSummary')}>
          <h2 className="text-lg font-semibold text-white mb-4">Financial Summary</h2>
          <span className="text-white text-xl">
            {collapsedSections.financialSummary ? '▼' : '▲'}
          </span>
        </div>
        {!collapsedSections.financialSummary && (
            ...
            <div className="flex items-center mb-2 space-x-4">
              <div className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <label htmlFor="showHuslanTable" className="text-white">Show Huslån</label>
              </div>
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => setSortBy('name')}
      ...
      {/* Daily Balance Chart */}
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleSection('dailyBalance')}>
          <h2 className="text-lg font-semibold text-white">Daily Balance</h2>
          <span className="text-white text-xl">
            {collapsedSections.dailyBalance ? '▼' : '▲'}
          </span>
        </div>
        {!collapsedSections.dailyBalance && (
          <>
      ...
      {/* Daily Balance Table */}
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-4 shadow-md">
        <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleSection('dailyBalanceTable')}>
          <h2 className="text-lg font-semibold text-white mb-2">Daily Balance Table</h2>
          <span className="text-white text-xl">
            {collapsedSections.dailyBalanceTable ? '▼' : '▲'}
          </span>
        </div>
        {!collapsedSections.dailyBalanceTable && (
          <div className="overflow-x-auto">
