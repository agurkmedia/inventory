...
export default function EconomyAndBudget() {
  ...
  const [selectedExpenses, setSelectedExpenses] = useState<{category: string, amount: number}[]>([]);
  ...
  const renderReceiptBreakdown = () => {
    if (!categorySummary.data || categorySummary.data.length === 0 || !categorySummary.data[0].expenses.breakdown) {
      return (
        <tr>
          <td colSpan={3} className="border px-4 py-2 text-center">No expense data available</td>
        </tr>
      );
    }
    ...
    return sortedData.map((item, index) => (
      <tr key={index}>
        ...
        <td className="border px-4 py-2">
          <button
            onClick={() => handleExpenseSelection(item)}
  ...
  const handleExpenseSelection = (expense: {category: string, amount: number}) => {
    setSelectedExpenses(prev => {
      const existingIndex = prev.findIndex(item => item.category === expense.category);
      if (existingIndex >= 0) {
  ...
  const renderReceiptStackedBarChart = () => {
    ...
    return (
      <div>
        ...
        {selectedExpenses.length > 0 && <SelectedExpensesSum />}
      </div>
    );
  };

  const SelectedExpensesSum = () => {
    const totalSum = selectedExpenses.reduce((sum, expense) => sum + expense.amount, 0);
    ...
    return (
      <div className="mt-4 p-4 bg-white bg-opacity-10 rounded-lg">
        ...
        <div className="mt-2 pt-2 border-t border-white flex justify-between text-white font-bold">
          <span>Total</span>
  ...
  return (
    <div className="space-y-4 h-full">
      ...
      {/* Add this search bar just before the Your Receipts section */}
      <div className="mt-8">
