...
export default function Items() {
  ...
  const handleViewCategory = async (categoryId: string, itemId: string) => {
    try {
  ...
  const handleSaveCategoryChange = async (itemId: string) => {
    try {
      ...
      if (selectedCategory) {
        await handleViewCategory(selectedCategory.id, itemId);
      }
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {inventory.items.map(item => (
                  <ItemCard key={item.id} item={item} onDelete={handleDeleteItem} onViewCategory={handleViewCategory} />
                ))}
              </div>
            </div>
          )}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {selectedCategory.items?.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((item) => (
                    <tr key={item.id} className={`border-b ${item.id === selectedCategory.highlightedItemId ? 'bg-yellow-200' : ''}`}>
                  {selectedCategory.items?.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((item) => (
                    <tr key={item.id} className={`border-b ${selectedCategory.highlightedItemId === item.id ? 'bg-blue-500 bg-opacity-50' : ''}`}>
