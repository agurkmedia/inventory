...
interface Item {
  ...
  inventoryId: string;
  image: string | null;
  // Add fields specific to ItemScraping
  productCode?: string;
  weight?: number;
  availability?: string;
  manufacturer?: string;
  sourceUrl?: string;
  isItemScraping: boolean; // To differentiate between Item and ItemScraping
}
...
export default function Items() {
  ...
  const fetchItems = async () => {
    try {
      const [itemsRes, scrapingsRes] = await Promise.all([
        fetch('/api/items'),
        fetch('/api/itemscrapings')
      ]);
      if (!itemsRes.ok || !scrapingsRes.ok) throw new Error('Failed to fetch items');
      const itemsData = await itemsRes.json();
      const scrapingsData = await scrapingsRes.json();
      
      const formattedItems = itemsData.map(item => ({ ...item, isItemScraping: false }));
      const formattedScrapings = scrapingsData.map(item => ({ ...item, isItemScraping: true }));
      
      setItems([...formattedItems, ...formattedScrapings]);
    } catch (err) {
