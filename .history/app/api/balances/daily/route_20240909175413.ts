...
import { addDays, addWeeks, addMonths, addYears, endOfMonth, startOfMonth, isEqual } from 'date-fns';
...
export async function GET(req: Request) {
  ...
  try {
    // Ensure the correct month range
    ...
    // Correct loop to ensure that the first day and last day are both included correctly
    for (let d = new Date(startDate); !isEqual(d, addDays(endDate, 1)); d = addDays(d, 1)) {
