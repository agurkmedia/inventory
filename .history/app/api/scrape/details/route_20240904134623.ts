...
export async function POST(req: Request) {
  try {
    ...
    for (const item of items) {
      ...
      const priceText = $('span.product-price').text().trim();
      const price = parseFloat(priceText.replace('kr', '').trim());
      const availability = $('span.product-availability').text().trim();
      const weightText = $('span.product-weight').text().trim();
      const weight = parseFloat(weightText.replace('Vekt', '').replace('kg', '').trim());
      const manufacturer = $('span.product-manufacturer').text().trim();
