...
export async function POST(req: Request) {
  try {
    ...
    for (const item of items) {
      ...
      // Extract price and manufacturer from dataLayer script
      ...
      const weightText = $('div.vsc-heading2 a[title]').text().trim();
      const weight = weightText ? parseFloat(weightText.replace('kg', '').trim()) : NaN;
