...
export async function POST(req: Request) {
  ...
  try {
    ...
    let totalAmountToAdd = 0; // Move the declaration here
    ...
    if (items && Array.isArray(items)) {
      ...
      const inventoryIds = Array.from(new Set(items.map((item: any) => item.inventoryId)));
      ...
      for (const receiptItem of receiptItemsData) {
        ...
        if (!existingReceiptItem) {
          ...
          // Sum up the totalAmount to add (include both positive and negative values)
          totalAmountToAdd += receiptItem.totalPrice;
        }
      }

      // Update the receipt's totalAmount using `set` to handle both increments and decrements
      if (totalAmountToAdd !== 0) {
        const newTotalAmount = receipt.totalAmount + totalAmountToAdd;

        receipt = await prisma.receipt.update({
          where: { id: receipt.id },
          data: {
            totalAmount: newTotalAmount, // Directly set the new totalAmount
          },
        });
      }
    }
