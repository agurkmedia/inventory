  ...
  try {
    ...
    const existingItems = await prisma.receiptItem.findMany({
      where: {
        ...
        date: { in: transactionDetails.map((t) => t.date) },
    ...
    const existingItemSet = new Set(
      existingItems.map(
        (item) => `${item.item.name}-${new Date(item.date).toISOString().split('T')[0]}-${item.totalPrice}`
      )
    );

    // Filter out transactions that already exist in the database
    const filteredTransactions = transactions.filter((transaction) => {
      const key = `${transaction.description.trim()}-${new Date(transaction.date).toISOString().split('T')[0]}-${Math.abs(transaction.amount)}`;
