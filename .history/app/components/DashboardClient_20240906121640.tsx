...
const DashboardClient = ({ dashboardData }) => {
  const { inventories, items, recentActivity } = dashboardData;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <DashboardCard title="Total Inventories" value={inventories.length} icon={<InventoryIcon />} />
        <DashboardCard title="Total Items" value={items.length} icon={<ItemIcon />} />
        <DashboardCard title="Low Stock Items" value={items.filter(item => item.quantity < 5).length} icon={<AlertIcon />} />
      </div>
