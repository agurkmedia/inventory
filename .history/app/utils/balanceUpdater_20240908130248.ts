...
export async function updateBalances(userId: string, startMonth: Date) {
  const endMonth = new Date(startMonth.getFullYear(), startMonth.getMonth() + 6, 1);  // Limit to next 6 months
  ...
  while (currentMonth < endMonth) {
    ...
    const remainingBalance = balance.startingBalance + totalIncome - totalExpenses - totalReceiptExpenses;
