...
export async function updateBalances(userId: string, startMonth: Date) {
  const currentDate = new Date(); // Add this line to define currentDate
  const tenYearsAgo = new Date(currentDate.getFullYear() - 10, 0, 1); // 0 represents January, 1 is the day
