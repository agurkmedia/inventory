...
export async function updateBalances(userId: string, startMonth: Date) {
  ...
  while (currentMonth <= twoYearsFromNow) {
    ...
    // If no balance exists for this month, create one using the carryover from the previous month
    if (!balance) {
      ...
      // Only update the starting balance if it doesn't match the carryover from the previous month
