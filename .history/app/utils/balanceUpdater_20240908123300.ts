...
export async function updateBalances(userId: string, startMonth: Date) {
  ...
  let previousRemainingBalance = 0; // Carry over the balance to the next month
    ...
    if (!balance) {
      balance = await prisma.balance.create({
        data: {
          ...
          year,
          month,
          startingBalance: previousRemainingBalance, // Carry over the balance from the previous month
